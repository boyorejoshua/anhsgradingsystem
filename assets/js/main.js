...
function renderHomeMenu(){
  ...
  const descriptions={
    instructions:'Read the step-by-step guide before using the system.',
    recordbook:'Set up classes, encode grades, and review analytics.',
    dailyatt:'Mark attendance for the selected class and date.',
    attsummary:'Review attendance records and class presence trends.',
    gradesummary:'View quarter totals, averages, and learner results.',
    consolidated:'Import subject JSON files and combine grade records.'
  };
  box.innerHTML=items.map(x=>`
    <button class="menu-bigbtn" onclick="showPage('${x.id}')">
      <span class="menu-bigicon">${x.i}</span>
      <span class="menu-bigtext">
        <strong>${x.l}</strong>
        <small>${descriptions[x.id]||'Open this section.'}</small>
      </span>
      <span class="menu-arrow">→</span>
    </button>
  `).join('');
  roleTitle.textContent=APP.role==='subject'?'Subject Teacher Menu':'Advisory Teacher Menu';
  roleDesc.textContent=APP.role==='subject'
    ?'Choose one task to continue. The page opens by itself so the teacher only focuses on one section at a time.'
    :'Choose one task to continue. Advisory teachers also have a separate page for consolidated grades.';
}

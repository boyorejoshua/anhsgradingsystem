...
function renderHomeMenu(){
  ...
  box.innerHTML=items.map(x=>`<button class="menu-bigbtn" onclick="showPage('${x.id}')">${x.l}</button>`).join('');
  roleTitle.textContent=APP.role==='subject'?'Subject Teacher Menu':'Advisory Teacher Menu';
  roleDesc.textContent=APP.role==='subject'?
    'Select Instructions, Record Book, Daily Attendance, Attendance Summary, or Grade Summary. Each one opens as its own page.':
    'Select Instructions, Record Book, Daily Attendance, Attendance Summary, Grade Summary, or Consolidated Grades. Each one opens as its own page.';
}
